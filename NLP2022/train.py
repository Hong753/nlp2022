...
def run(cfg):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            # Sample batch
            real_images, real_captions, real_caption_lengths = prepare_data(batch, device)
            batch_size = real_images.size(0)
            
            # Encode captions
            with torch.no_grad():
                hidden = text_encoder.init_hidden(real_captions.size(0))
                _, c = text_encoder(real_captions, real_caption_lengths, hidden)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                for batch in tqdm(test_loader, disable=True):
                    _, captions, caption_lengths = prepare_data(batch, device)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            save_path = "./runs/checkpoints/{}_dfgan/current_weights.pth".format(cfg.DATA.dataset_name)
            torch.save(checkpoint, save_path)
            if fid_score < best_fid_score:
                best_fid_score = fid_score
                best_checkpoint = checkpoint
                save_path = "./runs/checkpoints/{}_dfgan/best_weights.pth".format(cfg.DATA.dataset_name)
